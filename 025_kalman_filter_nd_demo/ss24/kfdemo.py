...
kf = kalmanfilter.kalman_filter( initial_x = P.initial_estimated_x,
                                 initial_P = P.initial_estimated_P,
                                 F = P.true_F,
                                 B = P.true_B,
                                 Q = P.true_Q,
                                 H = P.true_H,
...
while simu_time < P.max_simu_time:
    ...
    if int(simu_time) % 5 == 0:
        kf.correct_prediction_using_measurement( z )
        
    est_xs.append( kf.x )

    uncertainty_values.append( kf.get_scalar_measure_of_uncertainty_about_state() )
...
plt.subplot(nr_vis_rows,1, 5)
plt.title("Measure for uncertainty of KF estimates")
plt.xlabel("time [s]")
plt.ylabel("uncertainty [det|P|]")
plt.plot( uncertainty_values,  color="blue", label="KF uncertainty" )
plt.legend()
